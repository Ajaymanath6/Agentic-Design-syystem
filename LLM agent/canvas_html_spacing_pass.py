...
- **No new gratuitous spacing:** If the user prompt does **not** ask for spacing, padding, margins, gaps, or density, do **not** add new `p-*`, `m-*`, `gap-*`, or `space-*` utilities. If pass-1 HTML added theme padding that the prompt did not justify (e.g. `p-tight` on a bare label/title), remove those utilities for a leaner fragment.
...
- **Single-axis / surgical intent:** If the user mentions only one edge or region (bottom/top/left/right padding; margin below/above; gap between items in one list), adjust **only** the matching utilities on the relevant element(s) (`pb-*`, `pt-*`, `mb-*`, `mt-*`, `pl-*`, `pr-*`, `space-y-*`, `gap-*`, `last:pb-*`, etc.). Do **not** remove unrelated `p-*`, `m-*`, or `space-*` elsewhere unless the user asked to change those too.
- **Minimal change:** Prefer swapping a default numeric spacing class for a theme token on the same axis over stripping all padding from a parent.
- **Block / section padding:** If the user describes padding for a **whole section** or **group** with a named token, prefer **`p-*` / `px-*` / `py-*`** on the **container** wrapping that group over scattered **`pt-*` / `pl-*`** on inner elements, unless they specified a single edge.
- **Avoid double inset:** Padding on nested elements **stacks**. When an **ancestor** in the same card/workspace subtree already has theme padding (`p-*`, `px-*`, `py-*`, or `p-card-pad-*`), **remove** redundant overlapping padding classes from **inner** headings, labels, and title lines **unless** the user prompt clearly asks for a separate inner inset on that element. Goal: **single inset surface**—outer shell carries the padding; inner typography does not duplicate it unless specified.
- **Axis resolution:** When the user names a token and an axis **in a spacing request**, apply exactly the right prefix — `p-{{token}}` (all sides, or no side named in that request); `pt-` (top only); `pb-` (bottom only); `pl-` (left only); `pr-` (right only); `px-` (left+right / horizontal); `py-` (top+bottom / vertical). If the existing HTML has a wrong `pt-`/`pl-` combination but the user clearly meant "all sides", replace both with `p-{{token}}` on that element.
